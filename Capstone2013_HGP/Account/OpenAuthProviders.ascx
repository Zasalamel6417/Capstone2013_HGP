<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="OpenAuthProviders.ascx.cs" Inherits="Capstone2013_HGP.Account.OpenAuthProviders" %>
    ...
    <asp:ListView runat="server" ID="providersList" ViewStateMode="Disabled">
        <ItemTemplate>
            <button type="submit" name="provider" value="<%# HttpUtility.HtmlAttributeEncode(Item<ProviderDetails>().ProviderName) %>"
                ...
                <img src="<%# HttpUtility.HtmlAttributeEncode(Item<ProviderDetails>().ExtraData["Icon"]) %>"
                    alt="<%# HttpUtility.HtmlAttributeEncode(Item<ProviderDetails>().ProviderDisplayName) %>" />
            </button>
            <%--<img src="<%# HttpUtility.HtmlAttributeEncode(Item<ProviderDetails>().ExtraData["Icon"]) %>" 
                alt="<%# HttpUtility.HtmlAttributeEncode(Item<ProviderDetails>().ProviderDisplayName) %>" />--%>
        </ItemTemplate>
